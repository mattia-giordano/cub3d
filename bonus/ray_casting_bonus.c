...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/03/22 16:42:59 by mgiordan          #+#    #+#             */
/*   Updated: 2021/03/22 16:43:00 by mgiordan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		*get_slice(t_all *s, double ray, int *h)
{
	...
	*h = (int)(ceil(s->win.d_k / (hit->d * cos(RAD((ray - s->pl.angle))))));
	...
	i = 0;
	shade = 1.0 / hit->d * cos(RAD((ray - s->pl.angle))) * 500;
	free(hit);
	while (i < *h)
	{
		slice[i] = my_mlx_pixel_get(&tex.img, tex.col,
			(int)(i * ((float)tex.h / *h)));
		slice[i] = handle_shading(slice[i], shade);
		i++;
	}
	return (slice);
}
